-- opt frostseek module

local FrostSeek = _G.FrostSeek

local Options = {}
local _tk = FrostSeek and FrostSeek._v and FrostSeek._v.a("options", Options)
...
local function CreateModernCheckbox(parent, text, x, y)
    ...
    checkbox.bg:SetColorTexture(unpack(_tc("bgCheckbox")))
    
    checkbox.border = checkbox:CreateTexture(nil, "BORDER")
    checkbox.border:SetAllPoints()
    checkbox.border:SetColorTexture(unpack(_tc("border")))
    ...
    checkbox.check:SetVertexColor(unpack(_tc("primary")))
    checkbox.check:Hide()
    
    checkbox.highlight = checkbox:CreateTexture(nil, "HIGHLIGHT")
    checkbox.highlight:SetAllPoints()
    checkbox.highlight:SetColorTexture(unpack(_tc("bgRowHover")))
...
local function CreateSettingCheckbox(parent)
    ...
    checkbox.highlight = checkbox:CreateTexture(nil, "HIGHLIGHT")
    checkbox.highlight:SetAllPoints()
    checkbox.highlight:SetColorTexture(unpack(_tc("accentBar")))
    checkbox.highlight:Hide()
...
local function CreateCustomKeywordsTab(parent, scrollContent)
    ...
    local categories = {
        { id = "DUNGEON", name = "Dungeon", color = _tc("catDungeon"), desc = "Keywords that classify a message as a Dungeon run" },
...
local SETTINGS_CATEGORIES = {
    { id = "frostnetprofile", name = "Frostnet Profile", icon = "Interface\\AddOns\\FrostSeek\\Media\\texture\\bottoni\\generale.tga", settings = {
        ...
        { type = "checkbox", id = "frostnetEnabled", name = "Enable FrostNet", desc = "Enable the FrostNet network (join FSK channel and communicate)", default = true, getter = function() return FrostSeekDB.Settings.frostnetEnabled ~= false end, setter = function(v) FrostSeekDB.Settings.frostnetEnabled = v; local net = _G.FrostSeek and _G.FrostSeek.Network; if net then if v then net:JoinChannel(); net:JoinBLFGChannel() else net:LeaveChannel() end end; print("|cff88ccffFrostNet:|r " .. (v and "|cff44ff44Enabled|r" or "|cffff5555Disabled|r")) end },
    }},
...
local function CreateSettingControl(parent, setting, yOffset)
    ...
    -- Special type: FrostNet Role selector (creates its own frame)
    if setting.type == "frostnetrole" then
        ...
        for i, role in ipairs(roles) do
            ...
            btn:SetScript("OnClick", function(self)
                ...
                print("|cff88ccffFrostNet:|r Role set to " .. roleColors[self.roleName] .. self.roleName .. "|r")
            end)
...
StaticPopupDialogs["FROSTSEEK_THEME_RELOAD"] = {
    text = "FrostSeek: Apply theme |cffffffff%s|r?\n\nA UI reload is required to apply the new theme.",
    button1 = "Accept",
    button2 = "Cancel",
    timeout = 0,
    whileDead = true,
    ...
    OnAccept = function()
        ...
        if pending then
            ...
            print("|cff88ccffFrostSeek:|r Theme |cff88ccff" .. pending .. "|r applied. Reloading UI...")
            ...
            if editBox then
                ...
                print("|cff88ccffFrostSeek:|r Type |cffffffff/reload|r manually to apply the theme.")
            end
        end
    end,
    OnCancel = function()
        ...
        print("|cff88ccffFrostSeek:|r Theme change cancelled.")
    end,
}
...
if _G.FrostSeek and _G.FrostSeek.RegisterModule and _G.FrostSeek._v and _G.FrostSeek._v.c(_tk) then
    _G.FrostSeek:RegisterModule("options", Options)
end

if _G.FrostSeekTheme and _G.FrostSeekTheme.RegisterModule then
    _G.FrostSeekTheme.RegisterModule("options")
end
