...
local function CreateCleanEditBox(parent, width, height, isMultiLine)
    ...
    editBox:SetFontObject("FSKFontNormal")
...
local function CreateModernCheckbox(parent, text, x, y)
    ...
    local label = frame:CreateFontString(nil, "OVERLAY", "FSKFontNormalSmall")
...
local function CreateCustomMessageTab(parent, scrollContent)
    ...
    local title = frame:CreateFontString(nil, "OVERLAY", "FSKFontNormalLarge")
    title:SetPoint("TOP", frame, "TOP", 0, -15)
    title:SetText(L["options_custom_whisper"])
    title:SetTextColor(unpack(_tc("textAccent")))

    local desc = frame:CreateFontString(nil, "OVERLAY", "FSKFontNormalSmall")
    ...
    local templateLabel = frame:CreateFontString(nil, "OVERLAY", "FSKFontNormal")
    ...
    local varsHint = frame:CreateFontString(nil, "OVERLAY", "FSKFontNormalSmall")
    ...
    local previewLabel = frame:CreateFontString(nil, "OVERLAY", "FSKFontNormal")
    ...
    previewText = previewFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormal")
...
local function CreateCustomKeywordsTab(parent, scrollContent)
    ...
    for _, cat in ipairs(categories) do
        ...
        local catLabel = catFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormal")
        catLabel:SetPoint("TOPLEFT", 14, -8)
        catLabel:SetText(cat.name)
        catLabel:SetTextColor(cat.color[1], cat.color[2], cat.color[3])

        local catDesc = catFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormalSmall")
        ...
        local defaultsLabel = catFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormalSmall")
    ...
    local infoText = frame:CreateFontString(nil, "OVERLAY", "FSKFontNormalSmall")
...
local function GetSettingsCategories()
    ...
    { id = "theme", name = L["options_theme"], icon = "Interface\\AddOns\\FrostSeek\\Media\\texture\\bottoni\\sistema.tga", settings = {
        ...
        { type = "fontselector", id = "fontSelector", name = L["options_select_font_label"], desc = L["opt_select_font_desc"] },
    }}
}
end
...
local function CreateSettingControl(parent, setting, yOffset)
    ...
    if setting.type == "frostnetrole" then
        ...
        local roleLabel = roleFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormal")
    ...
    local nameLabel = controlFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormal")
    ...
    elseif setting.type == "slider" then
        ...
        local valueText = controlFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormal")
    ...
    elseif setting.type == "dropdown" then
        ...
        btn.arrow = btn:CreateFontString(nil, "OVERLAY", "FSKFontNormal")
    ...
    elseif setting.type == "category" then
        ...
        local title = categoriesFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormal")
        ...
        for i, category in ipairs(setting.categories or {}) do
            ...
            local label = catFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormal")
            label:SetPoint("LEFT", checkbox, "RIGHT", 8, 0)
    ...
    elseif setting.type == "themeselector" then
        ...
        local themeTitle = themeFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormal")
        ...
        local themeDesc = themeFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormalSmall")
    ...
    elseif setting.type == "fontselector" then
        controlFrame:Hide()
        nameLabel:SetText("")

        local fontFrame = CreateFrame("Frame", nil, parent)
        fontFrame:SetSize(580, 80)
        fontFrame:SetPoint("TOPLEFT", parent, "TOPLEFT", 10, yOffset - 20)

        local fontTitle = fontFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormal")
        fontTitle:SetPoint("TOPLEFT", fontFrame, "TOPLEFT", 0, 0)
        fontTitle:SetText(L["options_select_font_label"])
        fontTitle:SetTextColor(unpack(_tc("textPrimary")))

        local fontDropdown
        if UI and UI.CreateModernDropdown then
            fontDropdown = UI.CreateModernDropdown(fontFrame, 200, 24)
        else
            fontDropdown = CreateFrame("Frame", nil, fontFrame)
            fontDropdown:SetSize(280, 24)
        end
        fontDropdown:SetPoint("TOPLEFT", fontTitle, "BOTTOMLEFT", 0, -8)

        local fontDesc = fontFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormalSmall")
        fontDesc:SetPoint("TOPLEFT", fontDropdown, "BOTTOMLEFT", 0, -6)
        fontDesc:SetPoint("RIGHT", fontFrame, "RIGHT", 0, 0)
        fontDesc:SetText(L["opt_font_live_desc"])
        fontDesc:SetTextColor(unpack(_tc("textDim")))
        fontDesc:SetJustifyH("LEFT")

        local themeAPI = _G.FrostSeekTheme
        if themeAPI and themeAPI.GetFonts then
            local labels = themeAPI.GetFonts()
            if fontDropdown.SetOptions then
                fontDropdown:SetOptions(labels)
            end
            local currentLabel = themeAPI.GetFontLabel and themeAPI.GetFontLabel() or labels[1]
            if fontDropdown.SetText then
                fontDropdown:SetText(currentLabel)
            end
            fontDropdown.selectedValue = currentLabel

            fontDropdown.onChange = function(val)
                if themeAPI.SetFontByLabel then
                    themeAPI.SetFontByLabel(val)
                    if fontDropdown.SetText then
                        fontDropdown:SetText(val)
                    end
                end
            end
        end

        return fontFrame, -100
    ...
    elseif setting.type == "activityfilter" then
        ...
        for _, entry in ipairs(groups) do
            if entry.isHeader then
                ...
                local hText = hFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormal")
                ...
                toggleBtn.text = toggleBtn:CreateFontString(nil, "OVERLAY", "FSKFontNormalSmall")
                ...
                local label = row:CreateFontString(nil, "OVERLAY", "FSKFontNormalSmall")
    ...
    elseif setting.type == "header" then
        ...
        local headerText = headerFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormalLarge")
        ...
        if setting.desc and setting.desc ~= "" then
            local headerDesc = headerFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormalSmall")
...
function CreateOptionsWindow()
    ...
    local title = titleBar:CreateFontString(nil, "OVERLAY", "FSKFontNormalLarge")
    ...
    for _, category in ipairs(GetSettingsCategories()) do
        ...
        elseif category.id == "customkeywords" then
            ...
            local frameTitle = frame:CreateFontString(nil, "OVERLAY", "FSKFontNormalLarge")
    ...
    local footerText = footer:CreateFontString(nil, "OVERLAY", "FSKFontNormalSmall")
...
function Options:Initialize(parentFrame)
    ...
    self.title = self.frame:CreateFontString(nil, "OVERLAY", "FSKFontNormalLarge")
    self.title:SetPoint("TOP", self.frame, "TOP", 0, -20)
    self.title:SetText(L["options_system_settings"])

    self.desc = self.frame:CreateFontString(nil, "OVERLAY", "FSKFontNormal")
    ...
    local statusTitle = self.statusFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormal")
    ...
    self.statusText = self.statusFrame:CreateFontString(nil, "OVERLAY", "FSKFontNormalSmall")
