...
local function HookPresenceUpdates()
    ...
    Presence.HandlePresence = function(self, user)
        if FrostSeek and FrostSeek.SafeHandler then
            (FrostSeek.SafeHandler(orig))(self, user)
        else
            local ok, err = pcall(orig, self, user)
            if not ok and FrostSeek and FrostSeek.Logger then
                FrostSeek.Logger.Error("VersionCheck hook: " .. tostring(err))
            end
        end
