...
local ACTIVITY_FILTER_GROUPS = {
    ...
    { id = "DREAM", name = "Emerald Dream", keywords = {"DREAM", "NIGHTMARE"} },
...
-- Build reverse lookup: classified dungeon string -> filter ID
...
-- Export for Options module
LFG.ACTIVITY_FILTER_GROUPS = ACTIVITY_FILTER_GROUPS
LFG.ACTIVITY_DUNGEON_LOOKUP = ACTIVITY_DUNGEON_LOOKUP

-- Check if a classified (category, dungeon) passes the activity filter
function LFG.PassesActivityFilter(category, dungeon)
    -- If filter not initialized or empty, show everything (default = all on)
    if not FrostSeekDB.LFG.activityFilter then return true end

    -- PVP category: everything maps to "ARENA" filter id from ClassifyMessage
