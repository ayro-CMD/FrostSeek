...
local SPAM_WORDS = {
    ...
    "application", "roster spot", "core group", "core team",
    ...
    -- NOTE: Removed "up" (matches "Upper", "group", "setup" in legit LFG messages)
    -- NOTE: Removed "runs" (matches "LFM M+ runs" which is legitimate)
    -- NOTE: Removed "gold" (matches "Goldshire", "Mana Gold", and is caught by other filters)
    -- NOTE: Removed "bronze" (matches "BronzeLFG" addon name in messages)
    -- NOTE: Removed "auto" (matches "auto-invite", "auto-fill")
    -- NOTE: Removed "kick" (matches legit "kick" in LFG context)
    -- NOTE: Removed "doing" (matches "doing M+" which is legitimate)
    -- NOTE: Removed "stuff" (too broad, catches legit messages)
    -- NOTE: Removed "push" (matches "pushing keys" which is legitimate)
    -- NOTE: Removed "staff" (matches weapon type references)
    -- NOTE: Removed "apply" (too broad, "apply to group" is LFG context)
}
...
function LFG.IsLFMMessage(msg)
    ...
    -- [Keystone: ...] tag (used by BronzeLFG and players)
    if string.find(msg, "%[Keystone:") then
        return true
    end

    -- BronzeLFG abbreviated patterns in BLFG channel:
    -- "Need T" / "Need H" / "Need D" / "Need T/H" / "Need D?" etc.
    -- "LF T" / "LF H" / "LF D" / "LF 1D" etc.
    -- Also handles "Need Tank" / "Need Heal" / "Need DPS"
    ...
    -- LF1M, LF2M, LF3M, LF4M variants (e.g. "LF1M Tank")
...
-- Record a listing received from the FrostNet/BronzeLFG protocol into the LFG active searches
-- This bridges protocol-based listings (BLFG312~ / FSK1~) into the chat-based LFG display
function LFG.RecordFromListing(listing)
    ...
    -- Map listing type to LFG category
    ...
    -- Build a human-readable message from the listing data
    local msg = ""
    ...
    local isManastorm = (ltype == "Event")
    local isWorldBoss = (ltype == "World Boss")

    -- Update existing entry for same player or add new
    for _, record in ipairs(activeSearches) do
        if record.player == sender then
            record.message = msg
            record.lastUpdate = now
...
function LFG:Initialize(parentFrame)
    ...
    -- FrostNet button (opens Presence panel with online players info)
    self.frostnetBtn = CreateModernButton(self.controlsFrame, 100, 22, "FrostNet", _tc("accent"))
...
local CHANNEL_BLACKLIST = {
    ["LFG"] = true,
    [" LFG"] = true,
    -- FSK channel is protocol-only (FSK1~...), listings come via RecordFromListing() bridge
    ["FSK"] = true,
    -- BLFG channel carries BOTH protocol messages (BLFG312~...) AND human-readable LFG messages
    -- from BronzeLFG addon (e.g. "[Deadmines]: Need D?"). We do NOT blacklist BLFG so we can
    -- catch the human-readable messages. Protocol messages are filtered by IsAddonProtocolMessage().
}

local function IsAddonProtocolMessage(msg)
    if not msg then return false end

    -- FrostSeek / BronzeLFG protocol messages (FSK1~... BLFG312~...)
    if string.match(msg, "^FSK%d~") or string.match(msg, "^BLFG%d+~") then
        return true
    end
