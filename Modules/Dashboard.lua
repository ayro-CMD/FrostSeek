...
function Dashboard:Initialize(parentFrame)
    ...
    gsLabel:SetPoint("TOPLEFT", self.col3, "TOPLEFT", 20, -95)
    gsLabel:SetText("GearScore:")
    gsLabel:SetTextColor(0.8, 0.8, 0.8)
    
    self.gsValue = self.col3:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    self.gsValue:SetPoint("LEFT", gsLabel, "RIGHT", 10, 0)
    self.gsValue:SetText("0")
    self.gsValue:SetTextColor(0.6, 0.8, 1)
    
    local goldLabel = self.col3:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    goldLabel:SetPoint("TOPLEFT", self.col3, "TOPLEFT", 20, -120)
    ...
    sessionLabel:SetPoint("TOPLEFT", self.col3, "TOPLEFT", 20, -150)
    ...
    todayLabel:SetPoint("TOPLEFT", self.col3, "TOPLEFT", 20, -175)
...
function Dashboard:CalculateItemLevel()
    ...
    -- Calculate GearScore using FrostSeek's shared function
...
function Dashboard:UpdatePlayerStats()
    ...
    -- GearScore with color
    if cachedGS and cachedGS > 0 then
        self.gsValue:SetText(tostring(cachedGS))
        if FrostSeek and FrostSeek.GetGearScoreColor then
            local r, g, b = FrostSeek.GetGearScoreColor(cachedGS)
            self.gsValue:SetTextColor(r, g, b)
        else
            self.gsValue:SetTextColor(0.6, 0.8, 1)
        end
    else
        self.gsValue:SetText("0")
        self.gsValue:SetTextColor(0.6, 0.6, 0.6)
    end
    
    -- Gold TUTTO QUESTO E FATTO DA A-Y R.O
    local gold = GetMoney()
