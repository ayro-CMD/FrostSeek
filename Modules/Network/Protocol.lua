...
local function DedupKey(raw)
    ...
    elseif string.sub(stripped, 1, string.len(BLFG_PREFIX)) == BLFG_PREFIX then
        stripped = string.sub(stripped, string.len(BLFG_PREFIX) + 1)
    end
...
function Protocol:MarkProcessed(raw)
    ...
    for k, v in pairs(Protocol._dedupCache) do
        count = count + 1
        if (now() - v) > Protocol._dedupMaxAge then
...
function Protocol.Parse(raw)
    ...
    if string.sub(raw, 1, string.len(BLFG_PREFIX)) == BLFG_PREFIX then
        local parts = split(raw)
        if parts and #parts >= 2 and parts[1] == BLFG_PREFIX then
            local msgType = parts[2]
            return msgType, parts, "BLFG"
        end
    end
...
function Protocol.ConvertBLFGListing(parts)
    if not parts or #parts < 10 then return nil end
    local rolesNeeded = ""
    if parts[14] == "1" then rolesNeeded = rolesNeeded .. "Tank," end
    if parts[15] == "1" then rolesNeeded = rolesNeeded .. "Healer," end
    if parts[16] == "1" then rolesNeeded = rolesNeeded .. "DPS," end
    if rolesNeeded ~= "" then rolesNeeded = string.sub(rolesNeeded, 1, -2) end

    local ltype = parts[7] or "Dungeon"
    if ltype == "Key" or (parts[9] and string.find(parts[9], "Mythic", 1, true)) then
        ltype = "Key"
    end
    ...
    return {
        ...
        activity    = parts[8] or "",
        type        = ltype,
        difficulty  = parts[9] or "",
        leader      = parts[4] or "",
        roles       = rolesNeeded,
        minItemLevel = parts[11] or "",
        maxMembers  = tonumber(parts[13]) or 5,
        members     = tonumber(parts[12]) or 1,
        voice       = parts[17] or "None",
        loot        = parts[18] or "Group Loot",
        note        = parts[19] or "",
        key         = parts[10] or "",
        created     = tonumber(parts[20]) or now(),
        seen        = now(),
        _source     = "BLFG",
    }
end

function Protocol.ConvertBLFGPresence(parts)
    if not parts or #parts < 6 then return nil end
    return {
        name      = parts[3] or "",
        version   = parts[4] or "",
        level     = parts[5] or "",
        classFile = parts[6] or "",
        role      = parts[7] or "",
        zone      = parts[8] or "",
        guild     = parts[9] or "",
        seen      = tonumber(parts[10]) or now(),
        spec      = parts[11] or "",
        _source   = "BLFG",
    }
end

function Protocol.ConvertBLFGApplicant(parts)
    if not parts or #parts < 8 then return nil end
    return {
        listingId   = parts[3] or "",
        name        = parts[4] or "",
        class       = parts[5] or "",
        classFile   = parts[6] or "",
        level       = parts[7] or "",
        role        = parts[8] or "DPS",
        itemLevel   = parts[9] or "",
        roleType    = parts[10] or "",
        discord     = parts[11] or "No",
        note        = parts[12] or "",
        applied     = tonumber(parts[13]) or now(),
        _source     = "BLFG",
    }
end
...
Protocol.BLFG_PREFIX = BLFG_PREFIX
