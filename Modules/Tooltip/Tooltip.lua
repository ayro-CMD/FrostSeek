...
local function OnTooltipSetUnit(self)
    ...
    if not ilvl then
        local canInspect
        if FrostSeekCompat and FrostSeekCompat.CanInspect then
            canInspect = FrostSeekCompat.CanInspect(unit)
        else
            canInspect = CanInspect and CanInspect(unit)
        end
...
inspectFrame:SetScript("OnEvent", function(self, event, guid)
    ...
    if ilvl then
        ...
        if GameTooltip:IsShown() then
            local tName, tUnit = GameTooltip:GetUnit()
            if tUnit and UnitIsUnit(tUnit, unit) then
                GameTooltip:Hide()
...
if FrostSeekCompat and FrostSeekCompat.HookTooltipScript then
    FrostSeekCompat.HookTooltipScript("OnTooltipSetUnit", OnTooltipSetUnit)
else
...
autoInspectFrame:SetScript("OnEvent", function(self, event)
    ...
    if canInspect then
        ...
        if FrostSeekCompat and FrostSeekCompat.NotifyInspect then
            FrostSeekCompat.NotifyInspect(unit)
