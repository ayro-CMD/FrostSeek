...
local function EnsureSettingsIntegrity()
    ...
    if FrostSeekDB.LFG then
        if FrostSeekDB.LFG.filterWords == nil or FrostSeekDB.LFG.filterWords == "" then
            FrostSeekDB.LFG.filterWords = "echo,lfg,wts,buy,shop,gold,sell,account,boost,carry,pve,eu,na,need,wtt,wtb,bazar,hello,player"
        end
        if FrostSeekDB.LFG.maxMessageLength == nil then FrostSeekDB.LFG.maxMessageLength = 90 end
        if FrostSeekDB.LFG.frameDuration == nil then FrostSeekDB.LFG.frameDuration = 5 end
        if FrostSeekDB.LFG.popupCooldown == nil then FrostSeekDB.LFG.popupCooldown = 370 end
        if FrostSeekDB.LFG.maxConcurrentPopups == nil then FrostSeekDB.LFG.maxConcurrentPopups = 2 end
    end
...
--luigi
    if FrostSeekDB.LFG then
        if type(FrostSeekDB.LFG.customMessages) ~= "table" then
...
do
    local FSK_GUARD_NAMES = {
        "FSKFontNormalSmall", "FSKFontNormal", "FSKFontNormalLarge",
        "FSKFontNormalHuge", "FSKFontDisableSmall",
    }
    local FSK_GUARD_SIZES = {
        FSKFontNormalSmall = 10, FSKFontNormal = 12, FSKFontNormalLarge = 14,
        FSKFontNormalHuge = 20, FSKFontDisableSmall = 10,
    }
    local FSK_GUARD_BUNDLED = "Interface\\AddOns\\FrostSeek\\Media\\Fonts\\Carlito-Regular.ttf"
    -- shynga
    local FSK_GUARD_SRCS = {
    ...
    local _gFrame
    local function guardWorks(obj)
        if not obj then return false end
        local ok, file = pcall(function() return obj:GetFont() end)
        if not ok or not file or file == "" then return false end
        if not _gFrame then
            local okf, f = pcall(CreateFrame, "Frame", nil, nil)
            if okf and f then
                pcall(f.Hide, f)
                _gFrame = f
            end
        end
        if not _gFrame then return true end
        local okFs, fs = pcall(_gFrame.CreateFontString, _gFrame, nil, "BACKGROUND")
        if not okFs or not fs then return true end
        pcall(fs.SetFontObject, fs, obj)
        return pcall(fs.SetText, fs, "FrostSeek")
    end

    local function guardHealthy()
        for _, n in ipairs(FSK_GUARD_NAMES) do
            if not guardWorks(_G[n]) then return false end
        end
        return true
    end

    if not guardHealthy() then
        for _, n in ipairs(FSK_GUARD_NAMES) do
            local o = _G[n]
            if not guardWorks(o) then
                if not o then
                    local okc, created = pcall(CreateFont, n)
                    if okc and created then o = created end
                end
                if o then
                    _G[n] = o 
                    if not guardWorks(o) then
                        pcall(function() o:SetFont(FSK_GUARD_BUNDLED, FSK_GUARD_SIZES[n] or 12, "") end)
                    end
                    if not guardWorks(o) then
                        for _, s in ipairs(FSK_GUARD_SRCS) do
                            local so = _G[s]
                            if so then
                                local ok, f, _, fl = pcall(function() return so:GetFont() end)
                                if ok and f and f ~= "" then
                                    pcall(function() o:SetFont(f, FSK_GUARD_SIZES[n] or 12, fl or "") end)
                                    break
                                end
                            end
                        end
                    ...
                    if not guardWorks(o) then
                        pcall(function() o:SetFont("Fonts\\FRIZQT__.TTF", FSK_GUARD_SIZES[n] or 12, "") end)
                    end
                    if not guardWorks(o) then
                        pcall(function() o:SetFont("Fonts\\ARIALN.TTF", FSK_GUARD_SIZES[n] or 12, "") end)
                    end
                    if not guardWorks(o) then
                        pcall(function() o:SetFont("Fonts\\SKURRI.TTF", FSK_GUARD_SIZES[n] or 12, "") end)
                    end
                    if not guardWorks(o) then
                        pcall(function() o:SetFont("Fonts\\MORPHEUS.TTF", FSK_GUARD_SIZES[n] or 12, "") end)
                    end
                end
            end
        end
        ...
        if not guardHealthy() then
            print("|cff88ccffFrostSeek|r: no usable fonts on this client (bundled font missing?) - please report this message.")
        end
    end
end
...
local mainLogo = MainFrame:CreateTexture(nil, "OVERLAY")
pcall(function()
    mainLogo:SetTexture("Interface\\AddOns\\FrostSeek\\Media\\texture\\logo.tga")
    mainLogo:SetSize(38, 38)
    mainLogo:SetPoint("TOPLEFT", MainFrame, "TOPLEFT", 12, -10)
end)
FrostSeek.Logo = mainLogo
...
local function RegisterChatFilter()
    ...
    elseif not _G.FrostSeekOrigChatFrame_OnEvent then
