...
local function EnsureSettingsIntegrity()
    ...
    if FrostSeekDB.Profile then
        if FrostSeekDB.Profile.role == nil then FrostSeekDB.Profile.role = "" end
...
-- Tabs: 5 tab (dashboard, frostnet, lfg, lfm, options) — Profile is a sub-tab inside FrostNet
...
-- Minimap Button (existing, with FrostNet additions)
...
SlashCmdList["FSDEBUG"] = function()
    ...
    if Network then
        ...
        print("  BLFG Connected: " .. tostring(Network.isBLFGConnected))
        print("  BLFG ChannelId: " .. tostring(Network.blfgChannelId))
    end
...
local function LoadModules()
    C_Timer.After(1.0, function()
        local modulesToLoad = {
            "dashboard",
            "profile",     -- Profile must init before Listings (Profile is a sub-tab inside FrostNet)
            "lfg",
            "lfm",
            "listings",
            "options",
...
C_Timer.NewTicker(47, function()
    local v = FrostSeek._v
    if not v or not v.t then return end
    local cnt = 0
    for _ in pairs(v.r) do cnt = cnt + 1 end
    v.f = cnt < 3
end)
